...
# ============================================================
# PATCH HELPERS FOR LOCAL / CUSTOM SWEEP CONFIG
# ============================================================

def _first_value(x):
    """
    W&B/Hydra czasem przekazuje parametry jako:
      15
      [15]
      {"value": 15}
      {"values": [15]}
    Ta funkcja wyciąga z tego pojedynczą wartość.
    """
    if isinstance(x, dict):
        if "value" in x:
            return _first_value(x["value"])
        if "values" in x:
            return _first_value(x["values"])

    if isinstance(x, (list, tuple)):
        if len(x) == 0:
            return None
        return _first_value(x[0])

    return x


def _cfg_has(cfg, key):
    try:
        return key in cfg
    except Exception:
        try:
...
def _cfg_set(cfg, key, value):
    """
    Działa zarówno dla zwykłego dict, jak i wandb.config.
    """
    try:
        cfg[key] = value
        return
    ...
    try:
        cfg.update({key: value}, allow_val_change=True)
        return
    except Exception:
        pass

    raise RuntimeError(f"Nie udało się ustawić cfg[{key!r}] = {value!r}")


def _cfg_get_nested(cfg, keys, default=None):
    current = cfg
...
def patch_sweep_parameters(cfg):
    """
    PATCH:
    Zewnętrzne repo oczekuje parametrów sweepa jako top-level:
        cfg["segments_num"]
        cfg["clusters_num"]
        cfg["crf"]["alpha"] itd.

    U nas są one wygenerowane w:
        cfg["sweep"]["config"]

    Ta funkcja przenosi je w odpowiednie miejsce.
    """
    sweep_config = _cfg_get_nested(cfg, ["sweep", "config"], default=None)
    ...
    # segments_num
    # ----------------------------
    if not _cfg_has(cfg, "segments_num"):
        segments_num = _first_value(sweep_config.get("segments_num", None))
        if segments_num is not None:
            _cfg_set(cfg, "segments_num", int(segments_num))

    # ----------------------------
    # clusters_num
    # ----------------------------
    if not _cfg_has(cfg, "clusters_num"):
        clusters_num = _first_value(sweep_config.get("clusters_num", None))
        if clusters_num is not None:
            _cfg_set(cfg, "clusters_num", int(clusters_num))

    # ----------------------------
    # CRF params
    # ----------------------------
    crf_sweep = sweep_config.get("crf", None)

    if crf_sweep is not None:
        if not _cfg_has(cfg, "crf"):
            _cfg_set(cfg, "crf", {})

        for key, value in crf_sweep.items():
            try:
                cfg["crf"][key] = _first_value(value)
            except Exception:
                pass
...
def pipeline(cfg, subroot = "main"):
    ...
    # ------------------------------------------------------------
    # PATCH: make cfg["segments_num"] and cfg["clusters_num"]
    # available even if they came from cfg["sweep"]["config"].
    # ------------------------------------------------------------
    cfg = patch_sweep_parameters(cfg)

    # Set the sweep parameters that have to be the same
    log.info("STEP 0: SWEEP configiration")
    cfg['spectral_clustering']['K'] = int(cfg['segments_num'])
    cfg['multi_region_segmentation']['non_adaptive_num_segments'] = int(cfg['segments_num'])
    cfg['bbox']['num_clusters'] = int(cfg['clusters_num'])
    cfg['crf']['num_classes'] = int(cfg['clusters_num'])
