...
def load_dinov2_from_torchhub_without_local_shadow(model_name):
    """
    Loads official DINOv2 from torch.hub while temporarily hiding the local
    dinov2_with_attention_extraction package from Python import resolution.

    This avoids:
        ModuleNotFoundError: No module named 'dinov2.hub.cell_dino'

    which happens when local dinov2 shadows the torch.hub-downloaded dinov2.
    """
    import sys
    import torch
    ...
    try:
        ...
        for p in old_sys_path:
            p_str = str(p)
            if "dinov2_with_attention_extraction" in p_str:
                continue
...
def get_model(name: str):
    if 'dinov2' in name:
        ...
        model = load_dinov2_from_torchhub_without_local_shadow(name)
