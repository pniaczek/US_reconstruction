...
def crop_to_patch_grid(x, patch_size):
    """
    Crops tensor spatial size to a full DINO patch grid.
    Input:  (B, C, H, W)
    Output: (B, C, H_crop, W_crop), where H_crop and W_crop are divisible by patch_size.
    """
    if x.ndim == 3:
        x = x.unsqueeze(0)
    ...
    if h_crop < patch_size:
        h_crop = patch_size

    if w_crop < patch_size:
        w_crop = patch_size

    return x[:, :, :h_crop, :w_crop]


def pad_to_patch_multiple(x, patch_size, value=0.0):
    """
    Pads tensor spatial size to a multiple of DINO patch size.
    Input:  (B, C, H, W)
    Output: (B, C, H_pad, W_pad), where H_pad and W_pad are divisible by patch_size.
    """
    if x.ndim == 3:
        x = x.unsqueeze(0)
    ...
    if h + pad_h < patch_size:
        pad_h += patch_size - (h + pad_h)

    if w + pad_w < patch_size:
        pad_w += patch_size - (w + pad_w)
...
def _extract_multi_region_segmentations(
    ...
    # Reshape
    expected_1x = H_patch * W_patch
    expected_2x = H_patch * W_patch * 4

    print(
        "[DEBUG _extract_multi_region_segmentations] "
        f"clusters.size={clusters.size}, "
        f"H_patch={H_patch}, W_patch={W_patch}, "
        f"H_patch*W_patch={expected_1x}, "
        f"4*H_patch*W_patch={expected_2x}, "
        f"num_eigenvectors={num_eigenvectors}, "
        f"n_clusters={n_clusters}, "
        f"clustering1={clustering1}"
    )

    if clusters.size == expected_1x:
        segmap = clusters.reshape(H_patch, W_patch)

    elif clusters.size == expected_2x:
        segmap = clusters.reshape(H_patch * 2, W_patch * 2)

    else:
        # Obsługa siatki zdownsamplowanej, np. image_downsample_factor=2,4,...
        found = False

        for ds in [2, 3, 4, 5, 6, 7, 8]:
            h_ds = int(np.ceil(H_patch / ds))
            w_ds = int(np.ceil(W_patch / ds))

            if clusters.size == h_ds * w_ds:
                print(
                    "[DEBUG _extract_multi_region_segmentations] "
                    f"Detected downsampled grid: ds={ds}, "
                    f"h_ds={h_ds}, w_ds={w_ds}"
                )

                segmap_small = clusters.reshape(h_ds, w_ds)

                # Upsampling nearest-neighbor do rozmiaru patch-grid
                segmap = np.repeat(np.repeat(segmap_small, ds, axis=0), ds, axis=1)
                segmap = segmap[:H_patch, :W_patch]
        ...
        if not found:
            raise ValueError(
                "Cannot reshape clusters into segmentation map: "
                f"clusters.size={clusters.size}, "
                f"H_patch={H_patch}, W_patch={W_patch}, "
                f"H_patch*W_patch={expected_1x}, "
                f"4*H_patch*W_patch={expected_2x}"
...
def extract_bbox_features(
    ...
    utils.make_output_dir(str(Path(output_file).parent), check_if_empty=False)
    utils.make_output_dir(output_dir_crops, check_if_empty=False)
    ...
    if image_transform_data is None:
        raise ValueError("image_transform_data is None. It must be passed from extract_features.")
    ...
    for bbox_dict in tqdm(bbox_list):
        ...
        # Full image must match DINO patch grid.
        image = crop_to_patch_grid(image, patch_size)
        ...
        pos_x = pos_x.unsqueeze(0).to(device)
        pos_y = pos_y.unsqueeze(0).to(device)

        pos_x = pos_x.expand(-1, 3, -1, -1)
        pos_y = pos_y.expand(-1, 3, -1, -1)
        ...
        for i, (xmin, ymin, xmax, ymax) in enumerate(bboxes):
            xmin = max(0, min(int(xmin), image_w))
            xmax = max(0, min(int(xmax), image_w))
            ymin = max(0, min(int(ymin), image_h))
            ymax = max(0, min(int(ymax), image_h))
            ...
            mask = torch.from_numpy(binary_mask_rescaled)
            ...
            if apply_mask:
                image_crop = image_crop * mask_crop

            image_crop = pad_to_patch_multiple(image_crop, patch_size)
            pos_x_crop = pad_to_patch_multiple(pos_x_crop, patch_size)
            pos_y_crop = pad_to_patch_multiple(pos_y_crop, patch_size)
            mask_crop = pad_to_patch_multiple(mask_crop, patch_size)

            with torch.no_grad():
                features_crop = model(image_crop).squeeze().cpu()

                if C_pos == 0:
                    features_pos_x = torch.zeros_like(features_crop)
                    features_pos_y = torch.zeros_like(features_crop)
                else:
                ...
                if C_mask == 0:
                    ...
                    features_mask = model(mask_crop).squeeze().cpu()

                if feat_comb_method == "sum":
                    features = (
                        features_crop
                        + C_pos * features_pos_x
                        + C_pos * features_pos_y
                        + C_mask * features_mask
                    )
                    ...
                    raise ValueError(f"Unknown feat_comb_method: {feat_comb_method}")
        ...
        if len(features_crops) == 0:
            print(f"[WARN] No valid bboxes for image_id={image_id}")
            continue
...
def extract_semantic_segmentations(
    ...
    for bbox_dict in tqdm(bbox_list):
        ...
        # If segment 0 was skipped during bbox extraction, keep it as background.
        # If segment 0 was included, use its assigned cluster normally.
        if 0 not in semantic_map:
            semantic_map[0] = 0
