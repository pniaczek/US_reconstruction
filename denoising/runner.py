...
def denoise_case(case_name, methods=None):
    ...
    for method in methods:
        for path in files:
            ...
            if DENOISING_WRITE_BACK_TO_FOR_RECONSTRUCTION:
                output_case_names.append(output_case)

    return sorted(set(output_case_names))
...
def denoise_cases(case_names=None, methods=None):
    ...
    for case_dir in case_dirs:
        ...
        outputs = denoise_case(
            case_name=case_dir.name,
            methods=methods,
        )

        output_case_names.extend(outputs)

    output_case_names = sorted(set(output_case_names))

    if output_case_names:
        print("[OK] Denoising output cases:", output_case_names)

    return output_case_names